...
class TestCOCOScorer:
    ...
    annotation_data = {"annotations": [{"area": 447.64974501609544,
                                        "bbox": [1.0, 1.0, 2.0, 2.0],
                                        "category_id": 1,
                                        "id": 1,
                                        "image_id": 1,
                                        "iscrowd": 0}],
                       "images": [{"id": 1, "file_name": "dummy_img.tif",
                                   "width": 128.0, "height": 128.0}],
                       "categories": [{"id": 1, "name": "dummy_class_1"},
                                      {"id": 2, "name": "dummy_class_2"}]}

    @pytest.mark.parametrize("actual, predicted, annotation_data, expectation", [
        (dummy_actual, dummy_predicted, annotation_data, does_not_raise()),
        (dummy_actual, dummy_empty, annotation_data, does_not_raise()),
        (dummy_actual_test_len_mismatch, dummy_predicted, annotation_data,
         pytest.raises(ValueError,
                       match=r"Size mismatch between actual and predicted data")),
        (dummy_empty, dummy_empty, annotation_data,
         pytest.raises(ValueError,
    ...
    def test_basic_assertions_and_exceptions(self,
        ...
        from the utils file.
        """
        tmpfile = tempfile.NamedTemporaryFile(mode="w+", suffix=".json")
        json.dump(annotation_data, tmpfile)
        tmpfile.flush()
        json_filename = tmpfile.name
    ...
    @pytest.mark.parametrize("actual, predicted, annotation_data, stat_index, expectation", [
        ...
        (dummy_actual, dummy_predicted, annotation_data, 12,
         pytest.raises(IndexError,
                       match=r"index 12 is out of bounds for axis 0 with size 12"))
    ])
    def test_gt_and_predictions_validity(self,
                                         actual: Sequence[Sequence[Tuple[AxisAlignedBoundingBox,
                                                                         Dict[Hashable, Any]]]],
                                         predicted: Sequence[Sequence[Tuple[AxisAlignedBoundingBox,
                                                                            Dict[Hashable, float]]]],
        ...
        with expectation:
            ...
            assert (np.array_equal(np.concatenate([bbox.min_vertex, bbox.max_vertex]),
            ...
            assert all(np.concatenate([pred_bbox.min_vertex, pred_bbox.max_vertex])
