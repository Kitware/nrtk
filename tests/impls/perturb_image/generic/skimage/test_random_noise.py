...
class TestSaltNoisePerturber:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                pytest.raises(
                    NotImplementedError, match=r"Perturb not implemented for"
                ),
            ),
        ],
    )
    def test_no_perturbation(
        ...
        with expectation:
            perturber_assertions(
                perturb=SaltNoisePerturber(amount=0), image=image, expected=image
            )
    ...
    @pytest.mark.parametrize(
        ("rng", "amount"), [(42, 0.8), (np.random.default_rng(12345), 0.3)]
    )
    def test_configuration(
        self, rng: Union[np.random.Generator, int], amount: float
    ) -> None:
...
class TestPepperNoisePerturber:
    def test_consistency(self) -> None:
        ...
        perturber_assertions(
            perturb=inst.perturb, image=image, expected=EXPECTED_PEPPER
        )
    ...
    def test_no_perturbation(
        ...
        with expectation:
            perturber_assertions(
                perturb=PepperNoisePerturber(amount=0), image=image, expected=image
            )
...
class TestSaltAndPepperNoisePerturber:
    def test_consistency(self) -> None:
        ...
        inst = SaltAndPepperNoisePerturber(
            amount=amount, salt_vs_pepper=salt_vs_pepper, rng=rng
        )
    ...
    def test_configuration(
        self, rng: Union[np.random.Generator, int], amount: float, salt_vs_pepper: float
    ) -> None:
        """Test configuration stability."""
        inst = SaltAndPepperNoisePerturber(
            rng=rng, amount=amount, salt_vs_pepper=salt_vs_pepper
        )
        for i in configuration_test_helper(inst):
            assert i.rng == rng
            assert i.amount == amount
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                {"amount": 1.2},
                pytest.raises(
                    ValueError, match=r"SaltAndPepperNoisePerturber invalid amount"
                ),
            ),
            (
                {"amount": -0.2},
...
class TestGaussianNoisePerturber:
    ...
    def test_configuration(
        self, rng: Union[np.random.Generator, int], mean: float, var: float
    ) -> None:
        """Test configuration stability."""
...
class TestSpeckleNoisePerturber:
    def test_consistency(self) -> None:
        ...
        perturber_assertions(
            perturb=inst.perturb, image=image, expected=EXPECTED_SPECKLE
        )
...
EXPECTED_PEPPER = np.array(
    [[255, 0, 255], [255, 0, 255], [255, 255, 0]], dtype=np.uint8
)
...
EXPECTED_SPECKLE = np.array(
    [[255, 196, 255], [255, 144, 181], [255, 237, 254]], dtype=np.uint8
)
