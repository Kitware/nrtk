...
@pytest.mark.parametrize("factory_lengths, expectation", [
    ([], pytest.raises(ValueError, match=r"No factories passed in, cannot generate perturbations")),
    ([0], pytest.raises(ValueError, match=r"Factory passed in with invald length of 0")),
    ([1], does_not_raise()),
    ([1, 0], pytest.raises(ValueError, match=r"Factory passed in with invald length of 0")),
    ([1, 1], does_not_raise()),
    ([1, 2], does_not_raise()),
...
def test_gen_perturber_combinations(
    ...
    with expectation:
        ...
        factory_length_ranges = [range(factory_length) for factory_length in factory_lengths]
        ...
        for combination in cartesian_product:
            expected_output.append(list(combination))  # must convert to list because itertools.product returns tuple
