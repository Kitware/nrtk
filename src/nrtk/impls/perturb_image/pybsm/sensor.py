...
class PybsmSensor(Configurable):
    ...
    def __init__(
        ...
        if opticsTransmission is None:
            ...
            if opticsTransmission.shape[0] != optTransWavelengths.shape[0]:
                raise ValueError(
                    "opticsTransmission and optTransWavelengths must have the same length"
                )
        ...
        self.day = day
        self.pv = 0.0
        self.pvwavelength = 0.633e-6  # typical value
        self.Lx = D
        self.Ly = D
    ...
    def from_config(
        ...
        config_dict["optTransWavelengths"] = np.array(
            config_dict["optTransWavelengths"]
        )
        ...
        if opticsTransmission is not None:
            config_dict["opticsTransmission"] = np.array(
                config_dict["opticsTransmission"]
            )
