...
class JitterOTFPerturber(PerturbImage):
    def __init__(
        ...
        # limit for spatial frequency that the imaging s_ystem is able
        # to resolve is (1/rad).
        ...
        self.jitOTF = jitter_OTF(uu, vv, self.s_x, self.s_y)
    ...
    def perturb(
        ...
        if self.ifov >= 0 and self.slant_range >= 0:
            ...
            sim_img = resample_2D(blurimg, ref_gsd / self.slant_range, self.ifov)

        else:
            # Default is to set dxout param to same value as dxin
            psf = otf_to_psf(self.jitOTF, self.df, 1 / (self.jitOTF.shape[0] * self.df))
